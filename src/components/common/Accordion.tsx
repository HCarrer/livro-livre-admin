...
const Accordion = ({ title, content }: AccordionProps) => {
  ...
  return (
    <div className="w-full flex flex-col p-5 bg-soft-white rounded-[20px] drop-shadow-[0px_0px_10px_#00000020]">
      <button
        type="button"
        className="flex justify-between items-end text-navy-blue"
      ...
      <div
        className={`transition-all duration-200 ease-in-out overflow-hidden font-medium ${isOpen ? "max-h-96 mt-5" : "max-h-0"}`}
      >
        {typeof content === "string" ? (
          <p className="text-f7 text-default-grey">{content}</p>
        ) : (
          content
        )}
      </div>
