...
const TwoFactorStep = ({ handleClick }: { handleClick: () => void }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setValue("twoFactorCode", e.target.value, {
      shouldValidate: true,
      shouldDirty: true,
    });
  };
  ...
  return (
    <>
      <p className="text-navy-blue text-f6">
        Preencha os campos abaixo com os 6 dígitos que enviamos para o e-mail{" "}
        <span className="text-power-blue font-semibold">{emailValue}</span>
      </p>
      <Input
      ...
      <Button
        label="Continuar"
        variant="main"
        disabled={shouldDisableTwoFactorButton}
        type="submit"
        onClick={handleClick}
        className="w-full"
      />
